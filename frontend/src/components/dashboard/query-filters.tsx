  ...
  return (
    <div className="rounded-2xl border border-border bg-card px-3 py-3 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Lọc tài liệu truy vấn</p>
        {hasActiveFilters ? (
          <button
            type="button"
            onClick={resetFilters}
            disabled={disabled}
            className="text-xs font-medium text-muted-foreground underline decoration-border underline-offset-2 hover:text-foreground disabled:cursor-not-allowed disabled:text-muted-foreground/60"
          >
            Đặt lại
          </button>
        ) : null}
      ...
      {readyDocuments.length > 0 ? (
        <details className="mt-2 rounded-lg border border-border bg-background px-2 py-2">
      ...
      <details className="mt-2 rounded-lg border border-border bg-background px-2 py-2">
        <summary className="cursor-pointer text-xs font-medium text-foreground">Bộ lọc nâng cao</summary>

        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <div className="space-y-2">
            <p className="text-xs font-semibold text-muted-foreground">Loại tệp</p>
            <div className="flex flex-wrap gap-2">
              {FILE_TYPE_OPTIONS.map((option) => (
                <label
                  key={option.value}
                  className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-2 py-1 text-xs text-foreground"
                >
                  <input
                    type="checkbox"
                    className="h-3.5 w-3.5 accent-primary"
          ...
          <div className="space-y-2">
            <p className="text-xs font-semibold text-muted-foreground">OCR</p>
            <select
              value={ocrFilter}
              disabled={disabled}
              onChange={(event) => onOcrFilterChange(event.target.value as QueryOcrFilter)}
              className="h-8 w-full rounded-md border border-border bg-background px-2 text-xs text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:bg-muted"
            >
          ...
          <label className="space-y-1 text-xs text-muted-foreground">
            <span className="font-semibold text-muted-foreground">Từ ngày</span>
            <input
              type="date"
              value={uploadedAfter}
              disabled={disabled}
          ...
          <label className="space-y-1 text-xs text-muted-foreground">
            <span className="font-semibold text-muted-foreground">Đến ngày</span>
            <input
              type="date"
